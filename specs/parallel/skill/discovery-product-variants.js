import http from 'k6/http';
import { check, sleep } from 'k6';

//Define the stages for the test
export let options = {
    vus: 20000,
    duration: '20s',
};

// Set the base URL of the API
const baseUrl = 'https://stage-api.10minuteschool.net';


export default function () {

        let headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..9wQ8O63TaYsgj1wZLYh03F6m6Wo5NyJGAx726OqK5J8'
        }

        let res = http.get(`${baseUrl}/discovery-service/api/v1/products/ghore-boshe-spoken-english/variants`, {
            headers: headers
        });

        check(res, {
            'status is 200': (r) => r.status === 200,
            'status is 502': (r) => r.status === 502,
            'status is 503': (r) => r.status === 503,
            'status is 500': (r) => r.status === 500,
            'status is 520': (r) => r.status === 520,
            'status is 521': (r) => r.status === 521,
            'status is 522': (r) => r.status === 522,
            'response time > 200ms': (r) => r.timings.duration > 100,
        });

        if (res.status !== 200) {
            console.log("discovery-service/api/v1/products/ghore-boshe-spoken-english/variants > Status " + res.status);
        }

        sleep(1);
}


